...
namespace KB_ExtenV2 {
     ...
     */
    //% blockId=KB_ExtenV2_digitalRead
    ...
    */
    ...
    export function Motor_turn(Direction: motorTurn, Speed: number): void {
        if (Direction == motorTurn.Left) {
            KB_ExtenV2.MotorRun(1,1,0);
            KB_ExtenV2.MotorRun(2, 1, Speed);
        }
        else if (Direction == motorTurn.Right) {
            KB_ExtenV2.MotorRun(1, 1, Speed);
            KB_ExtenV2.MotorRun(2, 1, 0);
        }
    }
    ...
    */
    ...
    export function Motor_spin(Direction: motorTurn, Speed: number): void {
        if (Direction == motorTurn.Left) {
            KB_ExtenV2.MotorRun(1, 2, Speed);
            KB_ExtenV2.MotorRun(2, 1, Speed);
        }
        else if (Direction == motorTurn.Right) {
            KB_ExtenV2.MotorRun(1, 1, Speed);
            KB_ExtenV2.MotorRun(2, 2, Speed);
        }
    }
    ...
    */
    ...
    export function Motor(_ch: number, _Speed: number): void {
        if (_ch == 1) {
            if (_Speed >= 0) { KB_ExtenV2.MotorRun(motorSEL.M1, motorDIR.Forward, _Speed); }
            else { KB_ExtenV2.MotorRun(motorSEL.M1, motorDIR.Reverse, Math.abs(_Speed)); }
        }
        else if (_ch == 2) {
            if (_Speed >= 0) { KB_ExtenV2.MotorRun(motorSEL.M2, motorDIR.Forward, _Speed); }
            else { KB_ExtenV2.MotorRun(motorSEL.M2, motorDIR.Reverse, Math.abs(_Speed)); }
        }
    }
  ...
  */
    ...
    export function PID(_Speed: number, kp: number, kd: number, SensorRead: number[]): void {
        ...
        let errors = setpoint - KB_ExtenV2.Read_Position(SensorRead);
